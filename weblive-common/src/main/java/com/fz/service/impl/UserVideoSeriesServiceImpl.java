...
import com.fz.entity.query.UserVideoSeriesVideoQuery;
import com.fz.entity.query.VideoInfoQuery;
import com.fz.exception.BusinessException;
import com.fz.mappers.UserVideoSeriesVideoMapper;
...
import com.fz.mappers.UserVideoSeriesMapper;
import com.fz.service.UserVideoSeriesService;
...
public class UserVideoSeriesServiceImpl implements UserVideoSeriesService {
	...
	/**
	 * @description: 新增或修改视频列表
	 * @param bean 一个视频列表
	 * @param videoIds 视频列表中的视频
	 */
	@Override
	...
	public void saveUserVideoSeries(UserVideoSeries bean, String videoIds) {
		...
		if (bean.getSeriesId() == null){
			...
			// 新增分类中的视频信息(user_video_series_video)
	...
	private void checkVideoIds(String userId,String videoIds){
		...
		// 如果查询出来的数量和传入的数量不一致，说明传过来的参数有问题
	 ...
	 * @param userId  用户ID
	 * @param seriesId 分类ID
	 * @param videoIds  视频ID列表，多个视频用逗号分隔
	 * @return
	 * @author fz
	 * 2025/1/13 13:28
	 */
